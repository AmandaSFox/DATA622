...
The metrics from Assignment 2 are loaded and stored as a data frame for later comparison to the SVM models.
...
# write to csv as temp table
write_csv(df_all_metrics,"df_all_metrics_temp.csv")
```
  
## SVM RBF Kernel: Train and Test
  
An SVM with a radial basis function (RBF) kernel is also trained with 5-fold cross-validation. The cost and sigma parameters are selected by randomized search with precision used as the optimization metric.
...
# create final all metrics table
df_all_metrics <- bind_rows(df_metrics, df_svm_metrics)
df_all_metrics
